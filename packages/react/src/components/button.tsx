...
export function Button({ size = "medium", variant = "primary", className, children, asChild = false,...rest }: ButtonProps) {
  const classes = clsx(`r-btn r-btn--${size}`, {
    /**
  ...
  return (
    <Comp className={classes} {...rest}>{children}</Comp>
  )
}
