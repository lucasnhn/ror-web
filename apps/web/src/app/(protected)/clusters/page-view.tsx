...
//     const data = c.kubernetescluster?.spec?.data ?? {};
//     const state = c.kubernetescluster?.status?.state?.cluster ?? {};
//     const resources = state?.resources ?? {};
//     const price = state?.price ?? {};
//     const nodepools = state?.nodepools ?? [];

//     // serviceTags can be object or array; normalize to space-separated keys/values
//     const tagsRaw = data?.serviceTags as Record<string, unknown> | string[] | undefined;
//     const tags =
//       Array.isArray(tagsRaw)
//         ? tagsRaw.join(' ')
//         : tagsRaw && typeof tagsRaw === 'object'
//           ? Object.keys(tagsRaw).join(' ')
//           : '';
...
//       nodePoolCount: Array.isArray(nodepools) ? nodepools.length : null,
//       monthlyPrice: price?.monthly ?? null,
//       yearlyPrice: price?.yearly ?? null,
//       rorAgentVersion: state?.versions?.agent ?? '',
//       kubernetesVersion: state?.versions?.kubernetes ?? '',
//       nhnToolingVersion: state?.versions?.nhnTooling ?? '',
//       serviceTags: tags,
//     };
//   };
...
//   // Put this inside PageView, replacing your current exportableFromCluster
...
//     // --- versions is an array; pick by name (case-insensitive, partial supported)
//     const versions = state.versions ?? [];
//     const versionOf = (needles: string[]) => {
//       const lower = versions.map(v => ({
//         name: v?.name?.toLowerCase?.() ?? '',
//         version: v?.version ?? null,
//       }));
//       for (const n of needles) {
//         const hit = lower.find(x => x.name.includes(n));
//         if (hit) return hit.version;
//       }
//       return null;
//     };

//     // --- tags come from rormeta.tags (export values; fallback to key)
//     const tagsArr = (c as WithRorMeta)?.rormeta?.tags ?? [];
//     const serviceTags = Array.isArray(tagsArr)
//       ? tagsArr.map(t => t?.value ?? t?.key ?? '').filter(Boolean).join(' ')
//       : '';

//     // prefer spec count; fallback to status nodepools length if needed
//     const nodePoolCount =
...
//       rorAgentVersion: versionOf(['agent']),
//       kubernetesVersion: versionOf(['kubernetes', 'k8s']),
//       nhnToolingVersion: versionOf(['nhn tooling', 'nhntooling']),
...
//     try {
//       console.log('[Export] Export Filtered (CSV) starting');
...
//   // --- filtered Excel (SheetJS)
//   const handleExportFilteredExcel = async () => {
//     try {
//       console.log('[Export] Export Filtered (Excel) starting');
//       const rows = getFilteredClusterList().map(exportableFromCluster);
//       if (!rows.length) {
//         console.warn('[Export] No data to export');
//         return;
...
//       const XLSX = await import('xlsx'); // <-- dynamic import (no .default)
...
//       console.log('[Export] Excel file downloaded');
//     } catch (e) {
...
//       const s = String(v);
//       // Escape if needed (quotes, commas, or newlines)
//       if (/[",\r\n]/.test(s)) {
//         return `"${s.replace(/"/g, '""')}"`;
//       }
//       return s;
...
//     try {
//       console.log('[Export] Export All (CSV) starting');
...
//         h.length,
//         ...rows.map((r) => (r[h] == null ? 0 : String(r[h]).length)),
//       );
...
//     try {
//       console.log('[Export] Export All (Excel) starting');
//       const rows = safeClusters.map(exportableFromCluster);
//       if (!rows.length) {
//         console.warn('[Export] No data to export');
//         return;
...
//       // Build worksheet & apply column widths
//       const ws = XLSX.utils.json_to_sheet(rows);
//       ws['!cols'] = autosizeCols(rows);
