...
export const ClusterDetails = ({ user, className }: ClusterDetailsProps) => {
  ...
  // Guard to swallow the first onLayoutChange after a programmatic reset/apply.
  const applyingRef = useRef(false)

  const onLayoutChange = (layout: Layout[]) => {
    if (applyingRef.current) {
      applyingRef.current = false
      console.info(`${LOG_NS} onLayoutChange swallowed (post-reset)`, { bp: currentBreakpoint })
      return
    }
    setLayouts((prev) => ({
      ...prev,
