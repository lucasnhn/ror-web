...
export async function middleware(req: NextRequest) {
  ...
  for (const cookieName of possibleSessionCookies) {
    ...
    console.log(`[MIDDLEWARE] Trying token from cookie: ${cookieName}`)
    ...
    if (candidateToken) {
      ...
      console.log(`[MIDDLEWARE] Valid token found with cookie: ${cookieName}`)
      break
    } else {
      console.log(`[MIDDLEWARE] No valid token with cookie: ${cookieName}`)
    }
  }
  ...
  console.log(`[MIDDLEWARE] Token valid (cookie: ${usedCookieName}), proceeding`)
  return NextResponse.next()
}
