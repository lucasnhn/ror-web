...
const PUBLIC_ROUTES = ['/sign-in', '/sign-in-debug', '/auth-debug']
...
export async function middleware(req: NextRequest) {
  ...
  if (pathname.startsWith('/api/auth')) {
    console.log(`[MIDDLEWARE] Bypassing auth for /api/auth path: ${pathname}`)
    return NextResponse.next()
  }

  // Skip Dex callbackUrl redirects
  if (searchParams.has('callbackUrl')) {
    console.log(`[MIDDLEWARE] Bypassing auth for callbackUrl redirect`)
    return NextResponse.next()
  }
  ...
  console.log(`[MIDDLEWARE] Token found with keys: ${Object.keys(token).join(', ')}`)
  ...
  console.log(`[MIDDLEWARE] Token expiration check:`, {
    ...
    isExpired: exp < now,
    timeRemainingSec: Math.floor((exp - now) / 1000),
  })
...
export const config = {
  matcher: ['/((?!_next/|favicon.ico|robots.txt|sitemap.xml|api/auth|sign-in|sign-in-debug|auth-debug).*)'],
}
