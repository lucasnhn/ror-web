...
function isLayouts(value: unknown): value is Layouts {
  if (!value || typeof value !== 'object') return false
  return Object.values(value as Record<string, unknown>).every((v) => v === undefined || Array.isArray(v))
}

/** Safe deep clone to avoid RGL mutating our state/prefs objects */
const clone = <T>(v: T): T =>
  // structuredClone in modern browsers; JSON fallback otherwise
  typeof structuredClone === 'function' ? structuredClone(v) : JSON.parse(JSON.stringify(v))

/**
 * Persist and manage user-specific responsive layouts.
 */
export function useLayoutPreferences(key: LayoutKey, defaultLayouts: Layouts) {
  const isClient = typeof window !== 'undefined'

  const [pendingReload, setPendingReload] = useState<{ force: boolean } | null>(null)

  const reloadPage = useCallback(() => {
    if (typeof window === 'undefined') return
    setPendingReload({ force: true })
  }, [])

  useEffect(() => {
    if (!pendingReload) return

    if (!pendingReload.force) {
  ...
  const [layouts, _setLayouts] = useState<Layouts>(() => {
    if (!isClient) return clone(defaultLayouts)
    const prefs = getSavedUserPreferenceObject(PREFERENCES_KEY, DEFAULT_USERPREFERENCES)
    const saved = (prefs as Preferences)[key]?.layouts
    return isLayouts(saved) ? clone(saved) : clone(defaultLayouts)
  })

  const [layoutKey, setLayoutKey] = useState(0)
  const [currentBreakpoint, setCurrentBreakpoint] = useState('lg')

  /** Allow both value and functional updates; always deep-clone before storing */
  const setLayouts = (next: Layouts | ((prev: Layouts) => Layouts)) => {
    _setLayouts((prev) => {
      const v = typeof next === 'function' ? (next as (p: Layouts) => Layouts)(prev) : next
      return clone(v)
    })
  }

  const saveLayouts = (newLayouts: Layouts) => {
    const copy = clone(newLayouts)
    setLayouts(copy)
    updateUserPreferenceObject(PREFERENCES_KEY, { [key]: { layouts: copy } })
  }

  const resetToDefault = () => {
    // const copy = clone(defaultLayouts)
    // setLayouts(copy)
    // updateUserPreferenceObject(PREFERENCES_KEY, { [key]: { layouts: copy } })
    // setLayoutKey((k) => k + 1)
    reloadPage()
  ...
  return {
    layouts,
    setLayouts, // functional updater supported
    layoutKey,
    setLayoutKey,
    currentBreakpoint,
