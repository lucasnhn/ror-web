...
export function NavigationItem({ label, href, icon, items }: NavigationItemProps) {
  ...
  */
  ...
  const handleOnTriggerMouseOver = useCallback<MouseEventHandler<HTMLButtonElement>>((event) => {
      ...
      <Popover open={debouncedFlyoverOpen} onOpenChange={handleOnOpenChange}>
        <li className={s.item}>
          <PopoverTrigger asChild>
            <button className={s.trigger} onClick={handleOnTriggerClick} onMouseOver={handleOnTriggerMouseOver} onMouseLeave={handleOnTriggerMouseLeave}>
              {icon}
          ...
          <div className={clsxm(s.staticMenu, { [s.collapsed]: !shouldShowStaticMenu })}>
            <ul>
              {items.map((item) => (
                <li key={item.href}>
                  <Link href={item.href}>{item.label}</Link>
